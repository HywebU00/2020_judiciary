$(function() {
    ...
    if ($('header .megamenu').length > 0) {
        ...
        $(window).bind("load scroll resize", function(e) {
            ww = _window.outerWidth();
            if (ww >= wwMedium && $(this).scrollTop() > customHeight) {
                $('.header').addClass('fixed');
    ...
    // tabSet();
    // $('.peopletab>.tabItem:first-child>a').trigger('click');
    // tabSet1();
    ...
    $('.function_panel .share,.answer .share, .movie_content .share').find('li:last>a').focusout(function(event) {
    ...
    $('table').each(function(index, el) {
        //判斷沒有table_list
        if ($(this).parents('.table_list').length == 0 && $(this).parents('.fix_th_table').length == 0 && $(this).parent('form').length == 0 && $(this).parent('.table_sprite').length == 0) {
            $(this).scroltable();
        }
    });
